    ...
    return (
        <TouchableOpacity onPress={onPress} className="flex flex-col items-start w-60 h-80 relative">
            ...
            <View className="flex flex-row items-center
                bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5">
...
export const Card = ({item: {image, rating, name, address, price}, onPress}: Props) => {
    return (
        <TouchableOpacity onPress={onPress} className="flex-1 w-full mt-4 px-3 py-4 rounded-lg
         bg-white shadow-lg shadow-black-100/7- relative">
            <View className="flex flex-row items-center absolute
            px-2 top-5 right-5 bg-white-90 p-1 rounded-full z-50">
